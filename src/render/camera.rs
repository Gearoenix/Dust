...
impl<E> Base<E> where E: VectorElement {
    ...
    fn read(&mut self, s: &mut Stream) -> (Vec3<E>, Vec3<E>, E, E) {
        ...
        let rx:   E = num::cast(s.read(&0f32)).unwrap();
        let ry:   E = num::cast(s.read(&0f32)).unwrap();
        let rz:   E = num::cast(s.read(&0f32)).unwrap();
        ...
        let near: E = num::cast(s.read(&0f32)).unwrap();

        let mut screen_center: Vec3<E> = Vec3 {x: num::cast(0).unwrap(), y: num::cast(0).unwrap(), z: -near};
        screen_center = transform * screen_center;

        let mut screen_normal: Vec3<E> = Vec3 {x: num::cast(0).unwrap(), y: num::cast(0).unwrap(), z: near};
        screen_normal = transform * screen_normal;
        screen_normal.normalize();

        let mut screen_x_axis: Vec3<E> = Vec3 {x: num::cast(1).unwrap(), y: num::cast(0).unwrap(), z: num::cast(0).unwrap()};
        screen_x_axis = transform * screen_x_axis;
        screen_x_axis.normalize();

        let mut screen_y_axis: Vec3<E> = Vec3 {x: num::cast(0).unwrap(), y: num::cast(1).unwrap(), z: num::cast(0).unwrap()};
        screen_y_axis = transform * screen_y_axis;
        screen_y_axis.normalize();

        let screen_plane_constant = screen_center.dot(&screen_normal);

        let far:  E = num::cast(s.read(&0f32)).unwrap();
